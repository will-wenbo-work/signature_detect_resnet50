...
def train_and_export(data_dir, output_dir, num_epochs=10, lr=1e-3):
    device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")

    # load
    dataloaders, image_datasets = get_data_loaders(data_dir)
    ...
    model.fc = nn.Linear(num_features, len(class_names))  # 适配二分类
    ...
    torch.save(model.state_dict(), os.path.join(output_dir, "signature_resnet50.pth"))
    ...
    torch.onnx.export(
        model,
        dummy_input,
        os.path.join(output_dir, "signature_resnet50.onnx"),
        input_names=["input"],
        output_names=["output"],
